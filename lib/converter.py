...
class Converter(object):
    ...
    def __init__(self, filename, begin_offset=0):
        ...
        self.input = open(self.filename, 'rb').read()[self.begin_offset:]
        self.output = ''
        ...
        try:
            ...
            if start_envelope > 0:
                ...
                PrintUtils.print_info(
                    '.   offsets= 0x{0:X}-0x{1:X} | len= 0x{2:X} bytes'.format(self.begin_offset + offset,
                                                                               self.begin_offset + offset + start_envelope,
                                                                               start_envelope))
            ...
            PrintUtils.print_info(
                '.   [[XXXXX|ST_0xXX]] refers to known value in StringTable (extracted from in-band elements)')
            print()
